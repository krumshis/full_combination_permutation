...
class AllCombinations {
 ...
 private:
    ...
    int n_palindromes = 0;   // number of found palindrome
};
...
int main() {
    ...
    clock_t start = clock ();  // Time the solution.
    auto all_combinations = AllCombinations(n, &words);
    for (int r = 1; r <= n; r++) {
        all_combinations.CreateCombinations(r);
    }
    clock_t timeElapsed = ( clock() - start ) * 1000000.0/ CLOCKS_PER_SEC;
    cout << "Num of palindromes: " << all_combinations.GetPalindromeNumber();
    cout << "Computing palindromes took " << timeElapsed << " microseconds\n";
    return 0;
}
